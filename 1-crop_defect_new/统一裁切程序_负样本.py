...
# 统计负样本数量
negative_sample_lock = threading.Lock()
negative_sample_count = 0
max_negative_samples = 0  # 最大负样本数量限制
...
def increment_negative_sample():
    """线程安全地增加负样本计数，返回是否达到上限"""
    global negative_sample_count
    with negative_sample_lock:
        if negative_sample_count >= max_negative_samples:
            return False  # 已达到上限
        negative_sample_count += 1
        current_count = negative_sample_count
        
        # 检查是否达到上限
        if current_count >= max_negative_samples:
            stop_processing.set()  # 设置停止标志
            return True  # 刚达到上限
        return True  # 未达到上限，继续

def should_stop():
    """检查是否应该停止处理"""
    return stop_processing.is_set()
...
def has_defects_in_component(component_bbox, defects, overlap_thresholds, default_threshold):
    """
    检查部件区域内是否有缺陷（满足面积阈值要求）
    返回: True表示有缺陷（正样本），False表示无缺陷（负样本）
    """
    for defect in defects:
        defect_name = defect['name']
        original_bbox = defect['bbox']
        
        # 获取该类别的阈值
        threshold = overlap_thresholds.get(defect_name, default_threshold)
        
        # 计算交集比例
        ratio, _ = calculate_overlap_ratio(component_bbox, original_bbox)
...
def process_single_image(img_name, images_dir, image_index, component_ann_dir, defect_ann_dir, out_dir,
    ...
    for idx, component in enumerate(components, 1):
        ...
        # 检查该部件内是否有缺陷（正样本判断）
        has_defect = has_defects_in_component(
            comp_bbox_expanded, all_defects, 
        ...
        # 保存空XML（负样本，不包含任何object）
        out_xml_path = os.path.join(out_dir, "Annotations", new_xml_name)
        os.makedirs(os.path.dirname(out_xml_path), exist_ok=True)
        create_xml(new_img_name, crop_width, crop_height, [], out_xml_path)
        ...
        crop_info = {
            ...
            'defects_at_crop_time': [],  # 负样本无缺陷，保持结构一致
            'is_negative_sample': True  # 标记为负样本
        }
...
def start_multithread(images_dir, image_pkl_path, component_ann_dir, defect_ann_dir, out_dir,
    ...
    """
    global max_negative_samples
    max_negative_samples = negative_count

    print("=" * 70)
    print(f"🚀 开始处理负样本裁切 - 部件类型: {component_type}")
    print(f"📊 目标负样本数量: {negative_count}")
    print("=" * 70)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        with tqdm(total=negative_count, desc="负样本裁切进度", unit="个") as pbar:
            ...
            # 注意：即使 should_stop() 为 True，我们仍然处理完所有已提交的 futures
            # 因为它们可能已经在执行中，丢弃它们的结果会导致数据不一致
            # （文件已保存但JSON中没有记录）

    print(f"\n✅ 共处理了 {completed_count} 个任务，收集到 {len(all_crop_info)} 条裁切记录")

    # 保存裁切映射信息到JSON
    print(f"\n📝 开始保存JSON映射信息...")
    print(f"   all_crop_info 列表长度: {len(all_crop_info)}")

    crop_mapping = {}
    for xml_name, info in all_crop_info:
        crop_mapping[xml_name] = info

    print(f"   crop_mapping 字典长度: {len(crop_mapping)}")

    json_path = os.path.join(out_dir, "crop_mapping_负样本.json")
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(crop_mapping, f, ensure_ascii=False, indent=2)
    print(f"✅ 负样本裁切映射信息已保存到: {json_path}, 共 {len(crop_mapping)} 条记录")
    ...
    print("=" * 70)
    print(f"  目标负样本数量: {negative_count}")
    print(f"  实际裁切负样本: {negative_sample_count} 个")
    print(f"  完成率: {negative_sample_count/negative_count*100:.1f}%")
    print("=" * 70)
    
    if negative_sample_count < negative_count:
        print("\n⚠️  警告: 未达到目标负样本数量，可能数据集中负样本不足")
        print(f"  缺少: {negative_count - negative_sample_count} 个负样本")

# ==================== 主程序入口 ====================

def main():
    parser = argparse.ArgumentParser(
        description='负样本裁切工具 - 只裁切不包含缺陷的部件区域 - 支持pkl索引和文件夹扫描两种方式',
...
支持的部件类型: ddx(导地线), gt(杆塔), jyz(绝缘子), gd(挂点), global(全局负样本)
    ...
    print(f"  负样本数量: {args.negative_count}")
    print("=" * 70 + "\n")
