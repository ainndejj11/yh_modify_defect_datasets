...
1、总库裁切缺陷子图 —— `1-crop_defect_new/`

2、缺陷数据清洗后，标签映射回总库 —— `2-defect_mapping/`

把子图上人工清洗过的标注（增、删、改类别、调框）回写到原图总库。能确定的自动写，拿不准的进审核队列看图确认。

常规顺序：`1_生成变更集.py`（只读）→ `2_应用变更集.py`（默认预演，加 `--apply` 才写盘）→ `4_审核可视化.py`（网页决策）→ 再跑一次 `2_应用变更集.py` 写入补充变更集。`3_回滚.py` 只在出问题时用。

命令、参数和注意事项见该目录下的 [README](2-defect_mapping/README.md)。
